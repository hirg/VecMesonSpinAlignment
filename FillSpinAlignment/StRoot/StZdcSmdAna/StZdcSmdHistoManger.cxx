...
void StZdcSmdHistoManger::InitSub(int X_flag, int mode) // 0 for Same Event, 1 for Mixed Event
{
  // spin alignment analysis
  for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
	...
	h_mInvMass_phi[KEY_Mass2]->Sumw2();
      }
    }
  }
}

void StZdcSmdHistoManger::FillKstar(float pt, int Cent9, float CosThetaStar, float Res, float InvMass, double reweight, int X_flag, int mode)
{
  TString Mode[2] = {"SE","ME"};
  if(Res > 0.0)
  {
...
void StZdcSmdHistoManger::Fillphi(float pt, int Cent9, float CosThetaStar, float Res, float InvMass, double reweight, int X_flag, int mode)
{
  TString Mode[2] = {"SE","ME"};
  if(Res > 0.0)
  {
	...
	for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
	{
	  if(Cent9 >= vmsa::cent_low[i_cent] && Cent9 <= vmsa::cent_up[i_cent])
	  {
	    for(int i_theta = 0; i_theta < vmsa::CTS_total; i_theta++) // phi-psi2 bin
	    {
	      ...
	      {
		// flow
		TString KEY_Mass2 = Form("phi_pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_%s",i_pt,i_cent,i_theta,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
		h_mInvMass_phi[KEY_Mass2]->Fill(InvMass,reweight);
	      }
	    }
	  }
	}
...
void StZdcSmdHistoManger::WriteSub(int X_flag, int mode)
{
  TString Mode[2] = {"SE","ME"};
  // flow
  for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin
