...
void StVecMesonHistoManger::Init(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event
{
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
	...
	TString KEY_Mass2 = Form("pt_%d_Centrality_%d_EtaGap_0_CosThetaStar_%d_2nd_%s_%s",i_pt,i_cent,i_theta,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin
  {
	...
	TString KEY_Mass2_Spec = Form("Spec_pt_%d_%s_Centrality_%d_%s_%s_Sub",i_pt,pT[i_pT].Data(),i_cent,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
  ...
  {
    TString Mode[2] = {"SE","ME"};
    TString KEY_Mass2_Yields = Form("Yields_Centrality_%d_EtaGap_0_%s_%s_SysErrors_0",i_cent,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
  ...
  if(Res2 > 0.0)
  {
	...
	for(Int_t i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
	{
	  if(Cent9 >= vmsa::cent_low[i_cent] && Cent9 <= vmsa::cent_up[i_cent])
	  {
		...
		{
		  TString KEY_Mass2_Spec = Form("Spec_pt_%d_low_Centrality_%d_%s_%s_Sub",i_pt,i_cent,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
		  h_mMass_Spec[KEY_Mass2_Spec]->Fill(InvMass,reweight);
		}
		else
		{
		  TString KEY_Mass2_Spec = Form("Spec_pt_%d_high_Centrality_%d_%s_%s_Sub",i_pt,i_cent,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
		  h_mMass_Spec[KEY_Mass2_Spec]->Fill(InvMass,reweight);
		}
	      }
	    }
