...
void fitGainCorr(int energy = 6)
{
  string InPutFile = Form("/global/project/projectdirs/starprod/rnc/xusun/OutPut/AuAu%s/SpinAlignment/ZDCSMD/GainCorrPar/merged_file/file_%s_GainCorrPar.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str());
  // string InPutFile = Form("/Users/xusun/Data/SpinAlignment/AuAu%s/file_%s_GainCorrPar.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str());
  ...
  for(int i_eastwest = 0; i_eastwest < 2; ++i_eastwest)
  {
	...
	HistName = Form("h_mGainCorr%s%s_%d_ADC",vmsa::mEastWest[i_eastwest].c_str(),vmsa::mVertHori[i_verthori].c_str(),i_slat);
  ...
  for(int i_eastwest = 0; i_eastwest < 2; ++i_eastwest)
  {
    for(int i_verthori = 0; i_verthori < 2; ++i_verthori)
    {
      string CanName = Form("c_mGainCorr%s%s",vmsa::mEastWest[i_eastwest].c_str(),vmsa::mVertHori[i_verthori].c_str());
  ...
  float slopeEastVertical = ;
  float slopeEastHorizontal = ;
  float slopeWestVertical = ;
  float slopeWestHorizontal = ;
  ...
  string OutPutFile = Form("/global/project/projectdirs/starprod/rnc/xusun/OutPut/AuAu%s/SpinAlignment/ZDCSMD/GainCorrPar/merged_file/file_%s_GainCorrFac.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str());
  // string OutPutFile = Form("/Users/xusun/Data/SpinAlignment/AuAu%s/file_%s_GainCorrFac.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str());
