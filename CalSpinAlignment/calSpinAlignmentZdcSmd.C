...
void calSpinAlignmentZdcSmd(int energy = 6, int pid = 0, int year = 0)
{
  ...
  for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
  {
    for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
    {
      ...
      {
	string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SE",i_pt,i_cent,i_theta,vmsa::mPID[pid].c_str());
	...
	string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_ME",i_pt,i_cent,i_theta,vmsa::mPID[pid].c_str());
	...
	string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",i_pt,i_cent,i_theta,vmsa::mPID[pid].c_str());
  ...
  string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SE",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
  h_mMass_SE[KEY_SE_QA]->DrawCopy("PE");

  string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_ME",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
  ...
  string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
  ...
  for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
  {
    ...
    string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SE",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
    h_mMass_SE[KEY_SE_QA]->DrawCopy();

    string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_ME",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
    h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
    h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

    string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
    h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
    h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
    {
      ...
      {
	string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",pt_bin,i_cent,i_theta,vmsa::mPID[pid].c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
    ...
    string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::mPID[pid].c_str());
  ...
  if(pid == 0) // Polynomial fit subtraction is only needed for phi meson
  {
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
      {
	string KEY_theta = Form("pt_%d_Centrality_%d_1st_%s_SM",i_pt,i_cent,vmsa::mPID[pid].c_str());
	for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
	{
	  string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",i_pt,i_cent,i_theta,vmsa::mPID[pid].c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      string KEY_theta_QA = Form("pt_%d_Centrality_%d_1st_%s_SM",i_pt,vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  string KEY_theta_QA = Form("pt_%d_Centrality_%d_1st_%s_SM",vmsa::pt_QA[energy],vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
  {
    ...
    string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_1st_%s_SM",vmsa::pt_QA[energy],vmsa::Cent_start,i_theta,vmsa::mPID[pid].c_str());
    h_mMass[KEY_QA]->SetMarkerColor(1);
    h_mMass[KEY_QA]->SetMarkerStyle(24);
    h_mMass[KEY_QA]->SetMarkerSize(0.8);
    h_mMass[KEY_QA]->DrawCopy("pE");
  ...
  c_mMass_psi->cd(9);
  string KEY_Count = Form("Count_pt_%d_Centrality_%d_1st_%s_SM",vmsa::pt_QA[energy],vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  string KEY_BW = Form("BW_pt_%d_Centrality_%d_1st_%s_SM",vmsa::pt_QA[energy],vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  string KEY_Rho00_Count_QA = Form("Rho00_Count_Centrality_%d_%s",vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  string KEY_Rho00_BW_QA = Form("Rho00_BW_Centrality_%d_%s",vmsa::Cent_start,vmsa::mPID[pid].c_str());
  ...
  string figure_name = Form("../figures/rho00_pT_%s_ZDCSMD.eps",vmsa::mBeamEnergy[energy].c_str());
