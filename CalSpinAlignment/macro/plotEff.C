...
void PlotEff(Int_t mEnergy = 0, Int_t mPID = 0)
{
  TString InPutFile = Form("/Users/xusun/STAR/SpinAlignment/Data/AuAu%s/%s/Eff_%s.root",Energy[mEnergy].Data(),PID[mPID].Data(),Energy[mEnergy].Data());
  cout << InPutFile.Data() << endl;
  TFile *File_InPut = TFile::Open(InPutFile.Data());
  TProfile2D *p_mEfficiency = (TProfile2D*)File_InPut->Get("p_mEfficiency");

  TProfile *p_eff[pt_total];
  for(Int_t i_pt = 0; i_pt < pt_total; i_pt++)
  {
    TString ProName = Form("p_eff_pT_%d",i_pt);
    p_eff[i_pt] = (TProfile*)p_mEfficiency->ProjectionY(ProName.Data(),i_pt+1,i_pt+1);
  }
  ...
  for(Int_t i_pt = 0; i_pt < pt_total; i_pt++)
  {
    p_eff[i_pt]->SetMarkerStyle(Style[i_pt]);
    p_eff[i_pt]->SetMarkerColor(Color[i_pt]);
    p_eff[i_pt]->SetMarkerSize(1.1);
    p_eff[i_pt]->Draw("pEX0 same");
