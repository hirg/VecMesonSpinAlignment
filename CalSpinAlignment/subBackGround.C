	...
	for(int i_eta = vmsa::Eta_start; i_eta < vmsa::Eta_stop; i_eta++) // EtaGap loop
	{
	  for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	  {
		...
		if(i_theta == vmsa::CTS_start) h_mMass_theta[KEY_theta] = (TH1F*)h_mMass[KEY]->Clone(KEY_theta.c_str());
	      ...
	      f_sg->SetParameter(0,1.019);
	      f_sg->SetParLimits(0,1.014,1.024);
...
TF1* getSgFunc(int nFunc, int pid)
{
  ...
  if(nFunc == 1) f_func = new TF1("f_func",ParaBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],vmsa::FuncParNum[nFunc]);
  if(nFunc == 2) f_func = new TF1("f_func",LogaBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],vmsa::FuncParNum[nFunc]);
...
TF1* getBgFunc(int nFunc, int pid)
{
  ...
  if(nFunc == 1) f_func = new TF1("f_func",Para,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],vmsa::FuncParNum[nFunc]-3);
  if(nFunc == 2) f_func = new TF1("f_func",Loga,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],vmsa::FuncParNum[nFunc]-3);
